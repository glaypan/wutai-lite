      ...
      function handleLogin() {
        ...
        fetch("/api/auth/login", {
          ...
          .then(function (response) {
            if (response.ok) {
              // v7.6.0-P5: 用 fetch 跟随 303 后的最终 URL 跳转，保留 ctrlMode 参数（提示屏/字幕控制端）
              // 审查加强(2026-08-20 双模型): 同源校验 + /login 路径排除，防跨域/登录循环
              var targetUrl = "/?role=control";
              try {
                if (response.url && response.url.indexOf(location.origin) === 0 && response.url.indexOf('/login') < 0) targetUrl = response.url;
              } catch (e) {}
