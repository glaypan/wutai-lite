...
function buildEntryPortalHtml(config, passwordStatus, ports) {
  ...
  var operationCards = [
    {
      ...
      port: controlPort,
      path: '/?role=control',
      needsPassword: !!passwordStatus.control,
  ...
  // v7.6.0-P6: lite 恢复提示屏显示终端卡片（提示屏端口仍可通过 URL 直达，入口卡片补齐）
  if (screenPort > 0) {
    displayCards.push({
      icon: 'i-prompt',
      title: '提示屏',
  ...
  var sectionsHtml =
    renderSection('🛠️ 工作端', operationCards, '选择岗位进入对应工作台') +
    renderSection('🖥️ 显示终端', displayCards, '打开提示屏/字幕屏显示页面');
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      ...
      background-image:
        linear-gradient(135deg, #0b1026 0%, #1a1b4b 45%, #2a1a5e 75%, #14102e 100%),
        radial-gradient(circle at 18% 8%, rgba(64, 156, 255, 0.22), transparent 38%),
        radial-gradient(circle at 84% 16%, rgba(168, 121, 255, 0.18), transparent 34%),
    ...
    .card {
      ...
      position: relative;
      overflow: hidden;
      background-image: linear-gradient(150deg, rgba(45, 52, 110, 0.85) 0%, rgba(23, 25, 65, 0.92) 55%, rgba(28, 20, 60, 0.95) 100%);
      ...
      align-items: center;
      text-align: center;
      box-shadow: 0 8px 28px rgba(2, 6, 23, 0.45), inset 0 1px 0 rgba(255, 255, 255, 0.06);
